...
uint256 hashGenesisBlock("0x00000cafb4c9176c4119da09d006c241cdcd177a9077d50e584bafbe06a98486"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // hedgecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "hedgecoin Signed Message:\n";
...
{
    // hedgecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // hedgecoin: 1 day
static const int64 nTargetSpacing = 3 * 60; // hedgecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // hedgecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, hedgecoin - DarkGravity v2, written by Evan Duffield - evan@hedgecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
// For hedgecoin also add the protocol rule that the first output in the coinbase must go to the charity address and have at least 2.5% of the subsidy (as per integer arithmetic) (thx to einsteinium!)
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // hedgecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // hedgecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000769afdd66a38fabbe437d505edee57b74aab4d10aca17c10c18606e805a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "09/06/2015 HedgeCoin genesis block found!!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
/**/
	...
	block.print();	
        assert(block.hashMerkleRoot == uint256("0x0362104c942ad221297198fd90df216d471d1563bdb798d26945d90aa0b74baf"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc3, 0xb7, 0xd2 }; // hedgecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("hedgecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("hedgecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static hedgecoinMiner(CWallet *pwallet)
{
    printf("hedgecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hedgecoin-miner");
    ...
    try { loop {
        ...
        printf("Running hedgecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("hedgecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&hedgecoinMiner, pwallet));
}
